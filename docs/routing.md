...
```
홈 · 행사 · (갤러리 | 관리자) · 찬양팀 · 내정보
```

세 번째 칸은 `role === "admin"`이면 갤러리 대신 관리자 탭으로 바뀝니다. 각 탭은 `constants/theme.ts`의 `TAB_COLORS` 색과 `nav/creatures.tsx`의 캐릭터 아이콘을 갖고, 마우스를 따라 눈동자가 움직이며 랜덤하게 깜빡입니다.
