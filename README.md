...
### 개발 중 / 미연결
- **갤러리** — 준비 중 안내만 표시
- **회계 장부** — **iOS 앱으로 이관 예정**입니다. 관리자 페이지의 "재정 관리" 탭에 항목만 있고 누르면 준비 중 안내가 뜹니다. 웹 코드(`pages/accounting/`)는 라우터에 연결돼 있지 않고, 이 코드가 쿼리하는 `accounting_*` 테이블도 DB에 없습니다(현재는 `finance_*`)
- **통계** — 라우트 제거됨
